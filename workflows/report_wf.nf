...
workflow report_generation_full_wf {
    ...
    main:
        ...
        //idea for simplifed code:
            ...
            tool_list.each {
                if ( !params.${it}_off ) {
                    ${it}report = Channel.fromPath(workflow.projectDir + "/submodule/rmarkdown_reports/rmarkdown_reports/templates/${it}.Rmd", checkIfExists: true)
                    ${it}_report(${it}_report_ch.combine(${it}report))
                    //tool_list.add(${it}_report.out)
                    if ( !preceding_tool_active ) {
                        samplereportinput = ${it}_report.out //if its the first element of the list open a new channel containing the element
                        preceding_tool_active = True
                    }
                    else {
                        samplereportinput = samplereportinput.mix(${it}_report.out) //if its not the first list-element add it to the created channel
                    }
                }
            }
