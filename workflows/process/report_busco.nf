process busco_report {
        label 'ubuntu'  
    input:
        tuple val(name), val(version), val(db_version), val(command), val(plot_percentage_values), val(plot_absolute_values), path(result_file), path(markdown)
    output:
    ...
    script:
        ...
        sed -e 's|#PATHENV#|${params.output}/${name}/4.busco|g' | \
