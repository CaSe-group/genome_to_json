process json_report {
        ...
        label 'python3'
    input:
        tuple val(name), path(abricate_result), val(abricate_version), path(prokka_result), path(sourmash_result)
    output:
	    tuple val(name), path("*.json")
    script:
    if ( !params.abricate_off) { abricate_input = "${abricate_result},${abricate_version}" }
